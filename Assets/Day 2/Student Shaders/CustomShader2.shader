Shader "Custom/CustomShader2"
{
    Properties
    {
        ...
        _Intensity ("Intensity", Float) = 27
        _LigthDiffuse ("diffuse", Float) = 1
    }
  ...
  normalizedAngle += 1.;
  normalizedAngle *= .5;
  float radius = length(sin(Normal+(_Time.z)));
  half2 polarCoord = half2(radius,normalizedAngle);
  float lessThings = polarCoord.x+polarCoord.y;
  float lesserThings = frac(lessThings);
  float circo = length(half2(Normal.xy)-abs(sin(.5+(_Time.z))));
  float edge = 1. - smoothstep(0.,.01, abs(circo));
  half3 final = half3(lesserThings,lesserThings,lesserThings);
  //gl_FragColor = vec4(vec3(final),1.);
                ...
                float3 baseColery = col.rgb;
                litColor = litColor + baseColery;
                final = final * baseColery;
